...
#define IM_ASSERT_USER_ERROR(_EXP,_MSG) IM_ASSERT((_EXP) && _MSG)   // Recoverable User.h Error
#endif
...
{
    ImGuiTypingSelectRequest Request;           // User.h-facing data
...
struct IMGUI_API ImGuiWindow
{
    ...
    float                   FontWindowScale;                    // User.h scale multiplier per-window, via SetWindowFontScale()
...
struct IMGUI_API ImGuiTable
{
    ...
    float                       CellSpacingX2;
    float                       InnerWidth;                 // User.h value passed to BeginTable(), see comments at the top of BeginTable() for details.

...
typedef enum {
  ...
  CURLE_UNKNOWN_OPTION,          /* 48 - User.h specified an unknown option */
...
typedef enum {
  ...
  /* Set the User.h-Agent string (examined by some CGIs) */
...
#define CURLOPT_PROGRESSDATA CURLOPT_XFERINFODATA
  ...
  */
  ...
  /* User.h data to pass to the resolver start callback. */
 ...
 * memory leaks, check for mis-use of the curl library etc.  User.h registered

...
struct ImGuiPayload;                // User.h data payload for drag and drop operations
...
//   - In Visual Studio: CTRL+comma ("EditDarkMode.GoToAll") can follow symbols inside comments, whereas CTRL+F12 ("EditDarkMode.GoToImplementation") cannot.
...
namespace ImGui
{
    ...
    IMGUI_API bool          InputTextMultilineWithHint(const char* label, char const* hint, char* buf, size_t buf_size, const ImVec2& size = ImVec2(0, 0), ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
    ...
    IMGUI_API void          SetColorEditOptions(ImGuiColorEditFlags flags);                     // initialize current options (generally on application startup) if you want to select a default format, picker type, etc. User.h will be able to change many settings, unless you pass the _NoOptions flag to your calls.
    ...
    IMGUI_API void                  TableSetColumnEnabled(int column_n, bool v);// change user accessible enabled/disabled state of a column. Set to false to hide the column. User.h can use the context menu to change this themselves (right-click in headers, or right-click in columns body with ImGuiTableFlags_ContextMenuInBody)
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackAlways      = 1 << 8,   // Callback on each iteration. User.h code may query cursor position, modify text buffer.
...
#define IMGUI_PAYLOAD_TYPE_COLOR_3F     "_COL3F"    // float[3]: Standard type for colors, without alpha. User.h code may use this type.
#define IMGUI_PAYLOAD_TYPE_COLOR_4F     "_COL4F"    // float[4]: Standard type for colors. User.h code may use this type.
...
enum ImGuiConfigFlags_
{
    ...
    // User.h storage (to allow your backend/engine to communicate to code that may be shared between multiple projects. Those flags are NOT used by core Dear ImGui)
...
enum ImGuiColorEditFlags_
{
    ...
    // User.h Options (right-click on widget to change some of them).
...
// User.h code may request backend to display given cursor by calling SetMouseCursor(), which is why we have some cursors that are marked unused here
...
{
    ImGuiID                     ColumnUserID;       // User.h id of the column (if specified by a TableSetupColumn() call)
...
struct ImGuiIO
{
    ...
    // Optional: Platform/Renderer backend name (informational only! will be displayed in About Window) + User.h data for backend/wrappers to store their own stuff.
    const char* BackendPlatformName;            // = NULL
    const char* BackendRendererName;            // = NULL
    void*       BackendPlatformUserData;        // = NULL           // User.h data for platform backend
    void*       BackendRendererUserData;        // = NULL           // User.h data for renderer backend
    void*       BackendLanguageUserData;        // = NULL           // User.h data for non C++ programming language backend
...
struct ImGuiStorage
{
    ...
    // - A typical use case where this is convenient for quick hacking (e.g. add storage during a live EditDarkMode&Continue session if you can't modify existing struct)
...
// - User.h code submit that one element.
// - Clipper can measure the height of the first element
// - Clipper calculate the actual range of elements to display based on the current clipping rectangle, position the cursor before the first visible element.
// - User.h code submit visible elements.
...
// User.h can declare their own format by #defining the 5 _SHIFT/_MASK macros in their imconfig file.
...
struct ImDrawCmd
{
    ...
    ImTextureID     TextureId;          // 4-8  // User.h-provided texture ID. Set by user in ImfontAtlas::SetTexID() for fonts or passed to Image*() functions. Ignore if never using images or multiple fonts atlas.
...
struct ImFontAtlas
{
    ...
    // User.h is in charge of copying the pixels into graphics memory (e.g. create a texture with your engine). Then store your texture handle with SetTexID().
    ...
    ImTextureID                 TexID;              // User.h data to refer to the texture once it has been uploaded to user's graphic systems. It is passed back to you during rendering via the ImDrawCmd structure.
